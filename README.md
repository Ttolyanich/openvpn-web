...
2. **Настройка окружения:**
   ...
   cp .env.example .env
   ```
   Отредактируйте `.env` (например, через `nano .env`) и укажите параметры:
   * `PORT` — порт веб-панели (по умолчанию `5000`).
   * `SECRET_KEY` — уникальный секретный ключ для сессий Flask этой ноды. Можно сгенерировать на сервере командой: `openssl rand -hex 16`.
   * `CENTRAL_AUTH_URL` — адрес вашего сервера авторизации. Если он запущен на той же хост-машине (даже вне докера), используйте значение `http://host.docker.internal:5001` для автоматической пересылки трафика без прописывания IP.
   * `NODE_API_TOKEN` — токен авторизации ноды. **Он должен символ-в-символ совпадать с токеном `NODE_API_TOKEN`, указанным в файле `.env` на Сервере Авторизации.**
   * `NODE_NAME` — имя ноды для отображения во вкладке браузера (например, `VPN-Server-1`).
3. **Запуск контейнера:**
   Вы можете запустить панель напрямую, скачав готовый собранный образ из GitHub Container Registry (GHCR):
   ```bash
   docker compose up -d
   ```
   *Если вы хотите собрать образ локально из исходного кода самостоятельно, выполните команду:*
   ```bash
